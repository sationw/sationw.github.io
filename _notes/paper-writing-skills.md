...
```
Context 背景 ──→ Content 内容 ──→ Conclusion 结论
(Introduction)    (Results)       (Discussion)
提出问题/知识空白  数据/图表论证    解释意义/回应问题
```

```
段首句 ──→ 段中 ──→ 段末句
引入话题/设问  新内容/数据  给出本段结论
```
...
```
面包: Introduction ──→ 肉: Results ──→ 肉: Methods ──→ 面包: Discussion
     (提供背景)          (核心数据)      (可复现细节)     (解读意义/展望)
```
...
```
① 实验进行中 → ② 图表草图 → ③ 验证故事 → ④ 选定期刊 → ⑤ 写工作摘要
      ↓
⑥ 写 Results+Methods → ⑦ 写 Introduction → ⑧ 写 Discussion → ⑨ 精修图表
      ↓
⑩ 写 Abstract+Title → ⑪ 征求反馈 → ⑫ 投稿 → 审稿 → 修改 → 发表
```
...
```
作者写作顺序: 图表草图 → Results+Methods → Introduction → Discussion → 精修图表 → Abstract+Title
读者阅读顺序: Title → Abstract → Introduction → Results → Discussion → Methods
（写作"自下而上"构建论证，阅读"自上而下"消费结论，方向正好相反）
```
...
```
第1段: 领域背景与重要性 → 第2段: 收窄到本亚领域 → 第3段: 指出知识空白 gap → 最后段: 本文如何填补空白
       (宽泛已知)          (他人关键发现)        (为什么重要)           (方法特点+主要发现)
```
...
```
① Main message 主要信息 → ② Critical assessment 批判性评估 → ③ Comparison 与既往比较 → ④ Conclusions 结论
   直接回答引言问题         诚实指出局限/偏倚              先一致后矛盾文献          意义+未来方向
```
...
```
撰写完整初稿 → 按期刊要求格式化 → 投稿→编辑部初审 → 同行评审 → 修改稿 → 接收→校样→在线发表
(所有作者确认)  (Cover letter)   (是否送审)       (意见返回)  (逐条回复)
```
